...
|---|---|---|---|---|---|---|---|
| `claude-opus-4-7` | Completed; both contrasts reproduced | Reproduced; `B9J08_03708` via DIAMOND RBH (4,559 pairs) | Labeled volcano plots + PDF report | $131.83 | 23 | 169 | 7 |
| `claude-sonnet-4-6` | Completed; both contrasts reproduced | Reproduced; `B9J08_03708` via DIAMOND RBH | Volcano plots (PDF+PNG) + multi-page PDF | $24.87 | 20 | 165 | 16 |
| `gpt` (1st attempt) | Failed; stalled at data prep | None | None (history copy only) | unknown | 13 | 57 | 10 |
| `gpt2` (gpt-5.5) | Completed; both contrasts reproduced | Reproduced (after naive trap); `B9J08_03708` via DIAMOND RBH | 2 SVG + 2 PNG volcano plots + 3-page PDF | $23.46 | 27 | 191 | 11 |
| `gemini-2.5-pro` | Completed; both contrasts reproduced | Reproduced (sign inverted); `B9J08_03708` via local DIAMOND RBH | Text report + basic PDF; no volcano plot | $16.22 | 22 | 196 | 45 |
| `gemini-3.5-flash` | Completed; both contrasts reproduced | Reproduced (web-search mapping); `B9J08_03708` | 2 volcano plots + standalone PDF | $17.31 | 29 | 162 | 15 |
| `deepseek` | Completed; both contrasts reproduced | Reproduced (after brief trap); `B9J08_03708` via DIAMOND RBH | Notebook + HTML report + Galaxy page; no volcano, no PDF | $2.82 | 23 | 157 | 28 |
| `claude-haiku-4-5` | Failed; aborted in ~3 min | None | None (empty history) | unknown | 3 | 9 | 3 |

### Cost vs quality

Cost is an explicit efficiency axis, and because **all reported figures are LLM cost only** (Galaxy compute is free on usegalaxy.org and excluded throughout), they are directly comparable. The spread is the headline: a **~47× range in LLM spend** for the same scientific answer. `claude-opus-4-7` is the most thorough and complete run—correct, fully documented, labeled volcano PDFs—but at **$131.83** it cost ~5× the next run and ~5 hours of wall time. The mid-tier `claude-sonnet-4-6` ($24.87), `gpt2` ($23.46), `gemini-3.5-flash` ($17.31), and `gemini-2.5-pro` ($16.22) reached comparably correct endpoints at a fraction of that. `deepseek` reproduced the full result for **$2.82**—genuinely the cheapest, not an accounting artifact—though it shipped no volcano plot or PDF despite 10+ attempts. The takeaway: the extra LLM dollars bought completeness and polish (opus's labeled figures and final report) and less human shepherding, **not** scientific correctness—six runs converged on the same SCF1 result across a 47× LLM-cost range. Caveats remain on precision, not basis: `gpt2`'s figure is self-reported from its PDF, and `gpt`/`haiku` aborted too early to report a cost.

### Cross-attempt expression concordance

![SCF1 expression across attempts](scf1_expression_across_attempts.png)

**Figure 1.** Two views of the experiment across the six completing attempts. **(A)** SCF1 log2 fold-change for both contrasts, direction-normalized so down-in-mutant / down-in-AR0387 is negative. All six attempts reproduce the SCF1 collapse and cluster tightly at log2FC ≈ −6.8 (Fig. 1D) and ≈ −7.35 (Fig. 2D/S5); only `opus` is mildly attenuated (−5.83 / −6.35) because it reported raw DESeq2 MLE estimates rather than shrunken values. `sonnet` and `gpt2` are **bit-identical** (same `B9J08_03708`, baseMean, and log2FC to 12 significant figures) from running the unmodified IWC `rnaseq-pe`/`rnaseq-de` workflow with identical parameters; `gemini-2.5-pro` and `gemini-3.5-flash` reported the inverse sign (WT as numerator) and were normalized here. **(B)** LLM API cost (USD, log scale; Galaxy compute was free, so figures are directly comparable). Spend ranges ~47× — from $2.82 (`deepseek`) to $131.83 (`opus`) — for the same scientific answer; `haiku` and `gpt` (1st attempt) aborted at data prep and reported no cost.
...
### gpt (1st attempt) (OpenAI gpt-5.5, via Codex provider)
...
### gpt2 (OpenAI gpt-5.5, via Codex provider)
